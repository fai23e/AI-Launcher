...
-   **ウィンドウ管理と起動方法のカスタマイズ:**
    -   **起動方法の選択**: 設定ページで、サイトを「ポップアップウィンドウ」で開くか、「新しいタブ」で開くかを選択できます。
    -   **Shiftキーでの動作反転**: ランチャーで `Shift` キーを押しながらショートカットキーを入力すると、設定とは逆の動作でサイトを開きます。（例：設定が「ポップアップ」の場合、`Shift` + `C` でChatGPTが新しいタブで開きます）
-   **UI改善:**
    -   見やすく、使いやすいインターフェース。
...
-   **マニフェストバージョン:** 3
-   **主な使用API:** `chrome.commands`, `chrome.windows`, `chrome.runtime`, `chrome.storage`, `chrome.system.display`, `chrome.tabs`, `chrome.action`
-   **構成ファイル:**
